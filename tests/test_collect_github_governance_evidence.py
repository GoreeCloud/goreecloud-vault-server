...
class FakeClient:
    def __init__(self, overrides=None):
        self.responses = {
            ...
            "/repos/GoreeCloud/goreecloud-vault-server/actions/permissions": (
                200,
...
class GovernanceCollectorTests(unittest.TestCase):
    ...
    def test_actions_default_must_be_read_only(self):
        path = "/repos/GoreeCloud/goreecloud-vault-server/actions/permissions"
        with self.assertRaises(module.EvidenceError):
            self.collect({path: (200, {"default_workflow_permissions": "write"})})
