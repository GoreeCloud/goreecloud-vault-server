#!/usr/bin/env python3
"""Collect the Stable evidence governance object from GitHub without mutating settings."""
...
def collect(client: Any, repository: str, branch: str, release_environment: str) -> dict[str, Any]:
    ...
    required_checks_enforced = bool(checks) or bool(contexts)
    if not required_checks_enforced:
    ...
    _, actions_raw = client.get(f"/repos/{repository}/actions/permissions")
    actions = require_mapping(actions_raw, "actions_permissions")
